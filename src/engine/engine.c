...
void gsdl_init(const gsdl_init_info_t init, gsdl_props_t * props) {
    ...
    { // FPS + Other Setups
        ...
        props -> mb_used = -1;
            ...
            props -> desired_fps = 60;
            props -> desired_ms_delay = 1000 / 200;
        ...
        gsdl_create_phys_obj(&props -> render_texture_obj, mk_v2(0, 0), mk_v2(0, 0), init.w, init.h);
        gsdl_create_phys_obj(&props -> mouse, mk_v2(0, 0), mk_v2(0, 0), 16, 16); 
...
void gsdl_calc_cam_pos(gsdl_cam_t * cam, gsdl_props_t * props, gsdl_phys_obj_t * o, u16 tsx, u16 tsy, f64 dt) {
    ...
    cam -> x += (i32) round((o -> pos.x - cam -> x - ((w * 0.5) - (tsx * 0.5))) / 10 * dt);
    cam -> y += (i32) round((o -> pos.y - cam -> y - ((h * 0.5) - (tsy * 0.5))) / 10 * dt); 
}
...
void gsdl_add_entt_to_manager(gsdl_entt_manager_t * e_m, char * name) {
    gsdl_entt_t * e = calloc(1, sizeof(gsdl_entt_t));
    e -> component_data = ht_create(8);
    ht_insert(e_m, name, e);
}

void * gsdl_get_entt_comp_data(gsdl_entt_manager_t * e_m, char * e_name, char * c_name) {
    gsdl_entt_t * e = ht_get(e_m, e_name);
    void * data = ht_get(&e -> component_data, c_name);
    return data;
}
...
// game event string and codes must be in the same order
// ht_t gsdl_check_for_event(gsdl_props_t * props) {
void gsdl_start_event_handling(gsdl_props_t * props) {
    u32 event_num = props -> event_functions_num;
    char ** game_event_strings = props -> event_type_strings; 
    u32 * game_event_codes = props -> event_type_codes;
...
void gsdl_end_event_handling(gsdl_props_t * props) {
    ht_rm(&props -> selected_game_type_events);
}
